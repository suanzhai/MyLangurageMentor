...
class ScenarioTab(ABC):
    ...
    def get_scenario_description(self, scenario : ScenarioEnum):
        if scenario not in self.agents:
    ...
    def start_new_scenario_chatbot(self, scenario : ScenarioEnum, session_id : str):
        scenario_session_id = scenario.value + session_id
        initial_ai_message = self.agents[scenario].start_new_session(scenario_session_id)  # 启动新会话并获取初始AI消息
    ...
    def handle_scenario(self, user_input, chat_history, scenario, session_id : str):
        scenario_session_id = scenario + session_id
        bot_message = self.agents[ScenarioEnum(scenario)].chat_with_history(user_input, scenario_session_id)  # 获取场景代理的回复
        LOG.info(f"[ChatBot]: {bot_message}")  # 记录场景代理的回复
        return bot_message  # 返回场景代理的回复

    def scenario_change(self, scenario, session_id):
        if not session_id:
            session_id = str(uuid.uuid4())

        desc = self.get_scenario_description(ScenarioEnum(scenario))
        temp_chatbot, temp_state = self.start_new_scenario_chatbot(ScenarioEnum(scenario), session_id)
    ...
    def create_scenario_tab(self):
        with gr.Tab("场景"):  # 场景标签
            ...
            scenario_radio.change(
                fn=self.scenario_change,     # 更新场景介绍和聊天机器人
